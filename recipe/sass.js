...
module.exports = function (config, context) {
  ...
  Object.keys(entries).forEach(function (entry) {
    ...
    var output = path.join(config.static, entry + '.css')
    ...
    .then(result => {
      ...
      var inputRelPath = path.relative(config.client, input)
      var outputRelPath = path.relative(config.static, output)
...
function getEntries(config, context) {
  ...
  var files = context.entries.length > 0 ?
    context.entries : glob.sync(client + '/**/index.scss')
  files = files
